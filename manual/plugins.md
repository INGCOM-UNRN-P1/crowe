---
title: "Guía de Extensión y Creación de Plugins: crowe"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de crowe"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-crowe-plugins)=
# Guía de Extensión y Plugins: crowe

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`crowe`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-crowe-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`crowe` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para crowe`.
- **Punto de Entrada Oficial**: `crowe.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-crowe-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para crowe."""
    name = "custom_rule"
...
Para que `crowe` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."crowe.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `crowe` la reconozca:
...
(manual-crowe-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `crowe` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = crowe.ejecutar_analisis(
...
(manual-crowe-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `crowe` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar crowe
        run: pip install -e ./crowe
        
      - name: Ejecutar Auditoría
        run: crowe check src/ include/ --json > reporte.json
````

---

(manual-crowe-plugins-ejercicios)=
